...
const input = `[1518-02-28 00:47] falls asleep
...
const getParse = i =>
  i.split(/\n/).map(l => {
    ...
    return {
      ...
      hour,
      minute,
      originalTime: `${year}-${month}-${day} ${hour}:${minute}`,
...
const addTimeMostAsleep = i =>
  i.map(val => {
    const minutesAsleep = val.ranges.reduce((aslp, range) => {
      return (
        aslp +
  ...
  i
    ...
    .reduce((acc, val, index) => {
      ...
      if (guardIndex === -1) {
        ...
        acc[guardIndex].minutesAsleep += val.minutesAsleep;
      }
    ...
    .map(val => {
      ...
      const ranges = val.ranges.map(i => getMinutesInRange(i.start, i.end));
      const flat = ranges.reduce((a, v) => [...a, ...v], []);
      const duplicates = flat.reduce((a, v, i, s) => {
        const f = s.filter((x, ind) => i !== ind && x === v);
        if (f.length) a[v] = f.length;
        return a;
      }, {});
      return { ...val, duplicates };
    });

const getMostAsleepAndMinute = i =>
  i.reduce((acc, val) => {
    if (acc.minutesAsleep < val.minutesAsleep) {
      acc = val;
    }
    return acc;
  });

const getHightestDuplicate = i =>
  Object.keys(i.duplicates).length
    ? Object.keys(i.duplicates).reduce((a, v) =>
        i.duplicates[a] < i.duplicates[v] ? v : a
      )
    : undefined;

const getResult = i => {
  return getHightestDuplicate(i) * i.guardId;
};
/* part 2 */
const getMostDuplicates = i =>
  i.map(val => {
    return { ...val, highestDuplicate: getHightestDuplicate(val) };
  });
// .reduce((acc, val) => {
//   if (acc.highestDuplicate && acc.highestDuplicate < val.highestDuplicate)
//     acc = val;
//   return val;
// });

const getResult2 = i => parseInt(i.highestDuplicate) * i.guardId;

// const res = getMostAsleepAndMinute(
//   getMinuteMostAsleep(
//     addTimeMostAsleep(getRanges(getSort(getParse(testInput))))
//   )
// );

// console.log(res, "result =", getResult(res));

const res2 = getMostDuplicates(
  getMinuteMostAsleep(
    addTimeMostAsleep(getRanges(getSort(getParse(testInput))))
  )
);
...
console.log(
  ...
  // getResult2(
  res2
    .filter(a => a.highestDuplicate)
    .sort((a, b) => b.highestDuplicate - a.highestDuplicate)[0]
  // )
);

// console.log(
//   addTimeMostAsleep(getRanges(getSort(getParse(input)))).map(v => v.ranges)
// );
